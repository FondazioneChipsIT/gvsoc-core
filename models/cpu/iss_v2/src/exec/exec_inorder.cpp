...
void ExecInOrder::reset(bool active)
{
    if (active)
    {
        ...
        this->pc_set(this->bootaddr_reg.get());
...
void ExecInOrder::dbg_unit_step_check()
{
    ...
    {
        if (this->step_mode.get() && !this->debug_mode)
        {
            this->retain_inc();
            this->step_mode.set(false);
...
void ExecInOrder::exec_instr(vp::Block *__this, vp::ClockEvent *event)
{
    ...
    // already does at exec_instr_check_all): after a bridge force-resync/redirect
...
#endif
    {
        ...
        iss_reg_t next_pc = iss->exec.insn_exec_fast(insn, pc);
        if (iss->exec.is_insn_stalled)
        {
            iss->exec.is_insn_stalled = false;
...
void ExecInOrder::fetchen_sync(vp::Block *__this, bool active)
{
    ...
    if (!old_val && active)
    {
        _this->pc_set(_this->bootaddr_reg.get());
        _this->retain_dec();
    }
...
void ExecInOrder::bootaddr_apply(uint32_t value)
{
    ...
    this->iss.csr.mtvec.access(NULL, true, bootaddr);
