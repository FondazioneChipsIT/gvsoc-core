    ...
    : iss(iss)
{
    ...
    this->cycle.register_callback(std::bind(&Csr::mcycle_access, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    this->declare_csr(&this->time,     "time",      0xC01);
    this->time.register_callback(std::bind(&Csr::time_access, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    ...
    this->mcycle.register_callback(std::bind(&Csr::mcycle_access, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
    ...
    this->iss.new_master_port("time", &this->time_itf, (vp::Block *)this);

    this->mhartid = (this->iss.get_js_config()->get_child_int("cluster_id") << 5) | this->iss.get_js_config()->get_child_int("core_id");
...
void Csr::reset(bool active)
{
    if (active)
    {
        ...
        this->misa.value = this->iss.get_js_config()->get_int("misa");
...
bool Csr::tselect_access(iss_insn_t *insn, bool is_write, iss_reg_t &value)
{
    if (!is_write)
    {
        value = -1;
...
bool Csr::time_access(iss_insn_t *insn, bool is_write, iss_reg_t &value)
{
    uint64_t time;
...
bool Csr::mcycle_access(iss_insn_t *insn, bool is_write, iss_reg_t &value)
{
    if (!is_write)
    {
        value = this->iss.clock.get_cycles();
    }
    return false;
}
...
bool iss_csr_read(Iss *iss, iss_insn_t *insn, iss_reg_t reg, iss_reg_t *value)
{
  ...
  if (!getOption(iss, __priv_pulp)) {
    ...
    if (!iss->csr.access(insn, false, reg, *value))
    {
        return false;
    }
...
bool CsrAbtractReg::access(iss_insn_t *insn, bool is_write, iss_reg_t &value)
{
    bool update = true;
    for (auto callback: this->callbacks)
    {
        update &= callback(insn, is_write, value);
    }
...
bool Csr::access(iss_insn_t *insn, bool is_write, iss_reg_t address, iss_reg_t &value)
{
    CsrAbtractReg *csr = this->get_csr(address);
    if (csr != NULL)
    {
        return csr->access(insn, is_write, value);
    }
